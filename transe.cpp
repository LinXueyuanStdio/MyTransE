...
//	}
    ...
    aTrainList = (Triple *)calloc(aTripleTotal, sizeof(Triple)); //所有属性三元组
    aTripleTotal = 0;
...
//根据head和relation构造一个不同于head的corrupt_head
...
//根据tail和relation构造一个不同于tail的corrupt_tail
...
void* transetrainMode(void *con) {
    ...
    for (int k = a_transeBatch / transeThreads; k >= 0; k--) {
        ...
        int h1, t1, h2, t2,r;
        int EorV_flag=0;
        //随机构造head-batch或tail-batch的负例进行训练
        if (randd(id) % 1000 < pr) {
        ...
        if(float(randd(id)%1000)/1000.0 < combinationProbability[h1]) {
            int h1_cor = correspondingEntity[h1];//用跟h1对齐的实体计算loss
